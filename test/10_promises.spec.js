...
describe('Promises', () => {
  describe('Simple promises', () => {
    ...
    it('Turn this code into promise', (done) => {
      //Hint : the fetch request return a promise, how can we get the data out of it?
      const organisationName = 'Hackages';
      axios.get(`${url}/${organisationName}`)
        .then(organisation => {
          expect(organisation.data.email).to.equal('007@hackages.io');
          done();
        });
    });
  });
  ...
  describe('Promise.all', () => {
    ...
    it('What happens when a promise reject but is caught before getting to all when called with all?', (done) => {
      ...
      Promise.all(queue)
        .then((result) => {
          expect([1, 'error']).deep.equal(result);
          done();
        });
    });
